...
`mathematador-app/app.json`'s `"orientation": "portrait"` is a deliberate, permanent direction, not just an unreviewed default — see [issue #61](https://github.com/webstartapp/mathematador/issues/61) for the full reasoning. In short: every hub/menu screen is a single vertically-scrolling column of cards, the core minigame input (`DraggableKeyboard`, a 10-digit numeric pad) is laid out full-width at the bottom of the screen for one-handed thumb reach, and none of the ~10 candidate minigames in [#28](https://github.com/webstartapp/mathematador/issues/28) need landscape's extra width more than they'd lose from portrait's extra height. **Design new screens/minigames for portrait.** `mathematador-app/src/helpers/getScreenSizes.ts`, `HalvingLayout.tsx`, and `DraggableKeyboard.tsx`'s landscape branch exist only as defensive responsive-web handling (a browser window can be arbitrarily wide/short even with native orientation locked) — not a target to actively design toward. `intro.mp4` (the only one actually wired into a screen today, via `IntroScreen.tsx`'s `contentFit="cover"`) is landscape-format (16:9) source footage, crop-to-filled inside the portrait frame. `success.mp4`/`failure.mp4`/`levelup.mp4`/`itembought.mp4` are also landscape-format assets already sitting in `mathematador-app/assets/video/`, but aren't referenced anywhere in `mathematador-app/src` yet — not wired into any screen. Whenever they (or any future video) are wired in, request vertical/9:16 generations where practical rather than relying on `cover`-cropping landscape footage.
...
`mathematador-app`'s `start`/`android`/`ios`/`web` scripts all run through [`scripts/run-expo.js`](mathematador-app/scripts/run-expo.js) rather than calling `expo` directly. This is for testing on a physical device (native Expo Go, or a phone's browser over LAN) rather than the Expo web dev server itself: Expo Go's manifest request makes its own outbound HTTPS call, which fails with `unable to verify the first certificate` on a machine whose network does TLS-inspecting interception (common on corporate laptops/VPNs) unless Node also trusts the OS's own certificate store. The wrapper adds `NODE_OPTIONS=--use-system-ca` (which only *adds* the OS store to Node's trusted CAs — harmless on a machine without that kind of interception) but only when the running Node actually supports the flag (Node 23.8+) — passing it unconditionally via `NODE_OPTIONS` crashes older Node outright (including this repo's own CI image, pinned to Node 22), so the wrapper checks `process.versions.node` first rather than assuming everyone's on a new-enough Node.
