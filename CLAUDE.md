...
`mathematador-app`'s `start`/`android`/`ios`/`web` scripts all run through `cross-env NODE_OPTIONS=--use-system-ca`. This is for testing on a physical device (native Expo Go, or a phone's browser over LAN) rather than the Expo web dev server itself: Expo Go's manifest request makes its own outbound HTTPS call, which fails with `unable to verify the first certificate` on a machine whose network does TLS-inspecting interception (common on corporate laptops/VPNs) unless Node also trusts the OS's own certificate store. `--use-system-ca` only *adds* the OS store to Node's trusted CAs - it's a no-op on a machine without that kind of interception, so this is safe to leave on unconditionally.
